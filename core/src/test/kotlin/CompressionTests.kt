...
class CompressionTests {
    ...
    fun `same nodes different values`() {
        ...
        assertExactly(
            listOf(graph.getDifferenceVertex(node1)),
            metaNodes.find { graph.getDifferenceVertex(node1) in it.children }!!.children
        )
        assertExactly(
            listOf(graph.getDifferenceVertex(node3)),
            metaNodes.find { graph.getDifferenceVertex(node3) in it.children }!!.children
        )
